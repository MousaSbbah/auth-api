...
describe("when the user role is writer", () => {
  ...
  it("can not  access to put api/v2/food", async () => {
    const response2 = await request
      .put(`/api/v2/food/${id}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ name: "beard", calories: 100 });
  ...
  it("can not access  to delete an item api/v2/food/id", async () => {
    const response2 = await request
      .delete(`/api/v2/food/${id}`)
      .set("Authorization", `Bearer ${token}`);
    expect(response2.status).toEqual(403);
  });
  it("can not access to get all users api/v2/users", async () => {
    const response2 = await request
      .get(`/users`)
      .set("Authorization", `Bearer ${token}`);
    expect(response2.status).toEqual(403);
...
describe("when the user role is normal user", () => {
  let token;
  it("can not post api/v2/food", async () => {
    const response = await request.post("/signup").send(users[3]);
    token = response.body.token;
    const response2 = await request
      .post("/api/v2/food")
      .set("Authorization", `Bearer ${token}`)
  ...
  it("can not put api/v2/food", async () => {
    const response2 = await request
      .put(`/api/v2/food/${id}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ name: "beard", calories: 100 });
  ...
  it("can not  get all users api/v2/users", async () => {
    const response2 = await request
      .get(`/users`)
      .set("Authorization", `Bearer ${token}`);
    expect(response2.status).toEqual(403);
